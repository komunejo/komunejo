...
4. The space's own machinery (DEC-020, by default — never an option the owner must understand to accept): copy the operating kernel skills `${CLAUDE_PLUGIN_ROOT}/skills/entity-management` and `${CLAUDE_PLUGIN_ROOT}/skills/skill-manager` into the space's `.claude/skills/`, and install the validation hook from `${CLAUDE_PLUGIN_ROOT}/kernel/space-settings.json` as the space's `.claude/settings.json` — it fires the validator bundled inside the copied engine. The interviewer itself is not copied: it serves creation, not operation.
