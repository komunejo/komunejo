...
The marketplace is the owner’s own infrastructure, no third party: a `marketplace.json` catalog in a repository of the komunejo GitHub organization — `komunejo/komunejo-marketplace` — which users add with `/plugin marketplace add komunejo/komunejo-marketplace`; and, as the public face, the same catalog reachable from komunejo.net, addable by URL. Setting it up is an implementation task of v0.
